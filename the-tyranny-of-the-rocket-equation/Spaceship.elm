module Spaceship exposing (fuelRequiredForMass, adjustedFuelRequiredForMass, fuelRequiredForSpaceship)
...
fuelRequiredForMass mass =
	max	(mass // 3 - 2) 0

adjustedFuelRequiredForMass mass =
	let fuel = fuelRequiredForMass mass
	in
	case fuel of
		0 -> fuel
		_ -> fuel + (adjustedFuelRequiredForMass fuel)

fuelRequiredForSpaceship moduleMasses getFuelForModule =
	foldl (+) 0 (map getFuelForModule moduleMasses)

-- Solution for part one
fuelRequiredForSpaceship input fuelRequiredForMass

-- Solution for part two
fuelRequiredForSpaceship input adjustedFuelRequiredForMass
